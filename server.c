...
void gt_lcton_extnsn(const char *dirPath)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        else if (S_ISREG(fileStat.st_mode))
        {
            ...
            if (file_extension) {
                file_extension++;
                for (int i = 1; i < drs_usr_agmnt_dc; i++)
                {
...
void drs_p_rqst_to_clnt(int con_sd, int client_number_this)
{
    ...
    while (1)
    {
        ...
        localTime = localtime(&currentTime);
        strftime(strng_of_time, sizeof(strng_of_time), "%c", localTime);
        // pprinting the relevant client's command
        ...
        else if(strcmp("getfz", usr_agmnt_dv[0]) == 0 && drs_usr_agmnt_dc > 1)
        {
            const char *drctry_hme = getenv("HOME");
...
void main(int drs_agmnt_dc, char *drs_agmnt_dv[])
{
    ...
    if(drs_agmnt_dc > 0 && drs_agmnt_dc < 4)
    {
        printf("\nserver [server port] [mirror ip address] [mirror port]\n");
        exit(0);
    }
    long PORT = atol(drs_agmnt_dv[1]);
    char *drs_i_p_mrorr = drs_agmnt_dv[2];
    long prt_of_mrorr = atol(drs_agmnt_dv[3]);

    // finding the parent process id
    pid_t prnt_prcss = getpid();
    ...
    do
    {
        ...
        // client counter is being motinored and added
        nmbr_of_clnts++;

        // client number is being sent back to the respective client
        char client_number_string[16];
        snprintf(client_number_string, sizeof(client_number_string), "%d", nmbr_of_clnts);
        send(con_sd, &client_number_string, sizeof(client_number_string), 0);

        // displaying --> (time) client (num) connected
        currentTime = time(NULL);
        localTime = localtime(&currentTime);
        strftime(strng_of_time, sizeof(strng_of_time), "%c", localTime);
        printf("%s: Connected client %d\n", strng_of_time, nmbr_of_clnts);

        usleep(500000);

        //  determoning if the connection should be for the server or the mirror, depending on the client num
        if ((nmbr_of_clnts <= 4) || (nmbr_of_clnts > 8 && nmbr_of_clnts%2 == 1))
        {
            //  here, 'A' is sent for the server 
            //  'B' is sent for the mirror back to the client 
            char S_M = 'A';
            send(con_sd, &S_M, sizeof(S_M), 0);
            
            // closing unnecessary forks and forking the process client
            if (fork() == 0)
            {
                if(getppid() != prnt_prcss)
                {
                    exit(0);
                }
                else
                {
                    ...
                    drs_p_rqst_to_clnt(con_sd, nmbr_of_clnts);
                    exit(0);
                }
            }
        }
        ...
        {
            // 'B' means the mirror is to be connected, the ip and port is sent back to the respective client for its connection
            char S_M = 'B';
            send(con_sd, &S_M, sizeof(S_M), 0);
            usleep(500000);
            send(con_sd, &prt_of_mrorr, sizeof(long), 0);
            usleep(500000);
            send(con_sd, &drs_i_p_mrorr, sizeof(drs_i_p_mrorr), 0);
            
            // connection with the server is closed for the respective client
            close(con_sd);

            printf("%s: Redirected client %d to mirror server on port %ld\n", strng_of_time, nmbr_of_clnts, prt_of_mrorr);
        }
